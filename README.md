...
fetch → dedup → score → classify → format → send
  ...
  │        │       │        └─ 紧急分流 + 缓冲 + 跨源事件聚合
  │        │       └─ OpenClaw llm-task（HTTP / binary / lobster 三级 transport）
...
- OpenClaw **llm-task 插件**（安装脚本会自动启用；LLM 评分和摘要通过三级 transport 调用：HTTP 直连 > PATH binary > Lobster wrapper）
...
CrowWire 通过三级 transport 调用 OpenClaw `llm-task` 工具完成 LLM 评分，模型和 API 由 OpenClaw 配置决定。

**Transport 优先级：**

| 优先级 | 方式 | 环境变量 | 说明 |
|--------|------|----------|------|
| 1 | HTTP 直连 | `CLAWD_URL` + `CLAWD_TOKEN` | 推荐，最可靠 |
| 2 | HTTP 直连 | `OPENCLAW_GATEWAY_PORT` + `OPENCLAW_GATEWAY_TOKEN` | 自动拼接 `http://127.0.0.1:$PORT` |
| 3 | PATH binary | — | 检测 `clawd.invoke` / `openclaw.invoke` |
| 4 | Lobster wrapper | — | `lobster "clawd.invoke ..."` |
